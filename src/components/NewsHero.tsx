...
export default function NewsHero({ items, aiSummaries = {} }: Props) {
  ...
  return (
    <section className="border-b border-border pb-8 mb-8">
      <p className="text-xs font-semibold uppercase tracking-widest text-muted mb-4">Top Stories</p>
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        <div className="lg:col-span-3">
          <a href={lead.link} target="_blank" rel="noopener noreferrer" className="group block">
            <span className="text-xs font-medium uppercase tracking-wider text-muted">{lead.category}</span>
            ...
            <div className="flex items-center gap-3 text-xs text-muted flex-wrap">
              ...
              <span className={`font-medium ${
                lead.marketImpact.startsWith("Bullish") ? "text-green-700"
                : lead.marketImpact.startsWith("Bearish") ? "text-red-700"
                : "text-muted"
              }`}>{lead.marketImpact}</span>
            </div>
          </a>
        </div>
        <div className="lg:col-span-2 flex flex-col gap-6 lg:border-l lg:border-border lg:pl-8">
          {secondary.map((item, i) => (
            <a key={i} href={item.link} target="_blank" rel="noopener noreferrer" className="group block">
              <span className="text-xs font-medium uppercase tracking-wider text-muted">{item.category}</span>
